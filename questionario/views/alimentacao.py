...
from ..data.data_alimentacao import ALIMENTACAO_DOMINIOS, ALIMENTACAO_OPCOES, ALIMENTACAO_CLASSIFICACAO
from ..services import notificar_terapeuta

TOTAL_PAGINAS = len(ALIMENTACAO_DOMINIOS)

_URL_NAMES = {
    "url_form_name": "alimentacao_form",
    "url_publico_name": "alimentacao_publico",
    "url_visualizar_name": "alimentacao_visualizar",
    "url_resultado_name": "alimentacao_resultado",
    "avaliacao_titulo": "Avaliação de Alimentação",
}
...
def _processar_post_pagina(request, dom):
    ...
    valores_validos = {o["valor"] for o in ALIMENTACAO_OPCOES}
    erros, novas, obs = [], {}, {}
...
def nova_avaliacao_alimentacao(request, paciente_id):
    ...
    if not hasattr(request.user, 'perfil') or not request.user.perfil.tem_acesso('alimentacao_seletiva'):
        ...
        messages.error(request, "Você não tem acesso ao módulo Triagem de Alimentação.")
    ...
    return redirect("alimentacao_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def alimentacao_form(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoAlimentacao, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status == "concluida":
        return redirect("alimentacao_resultado", avaliacao_id=avaliacao_id)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("alimentacao_form", avaliacao_id=avaliacao_id, pagina=1)

    dom = ALIMENTACAO_DOMINIOS[pagina - 1]
    numeros = [n for n, _, _reverso in dom["itens"]]
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar_incompleto:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                return redirect("alimentacao_resultado", avaliacao_id=avaliacao_id)
            return redirect("alimentacao_form", avaliacao_id=avaliacao_id, pagina=proxima)
    ...
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html", {
        **_URL_NAMES,
        ...
        "paginas_dominios": [(i + 1, ALIMENTACAO_DOMINIOS[i]["nome"]) for i in range(TOTAL_PAGINAS)],
        "pagina_anterior": pagina - 1 if pagina > 1 else None,
        "itens_faltando": itens_faltando,
    })


def alimentacao_publico(request, token, pagina):
    avaliacao = get_object_or_404(AvaliacaoAlimentacao, token=token)
    if avaliacao.status == "concluida":
        return render(request, "questionario/dashboard/concluido.html")
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("alimentacao_publico", token=token, pagina=1)
        ...
        if erros and not confirmar_incompleto:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                try:
                    ...
                    pass
                return render(request, "questionario/dashboard/concluido.html")
            return redirect("alimentacao_publico", token=token, pagina=proxima)
...
def alimentacao_resultado(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoAlimentacao, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status != "concluida":
        return redirect("alimentacao_form", avaliacao_id=avaliacao_id, pagina=avaliacao.pagina_atual)
    resultado = []
    total_score = 0
    total_max = 0
    for dom in ALIMENTACAO_DOMINIOS:
        ...
        resultado.append({
            ...
            "max": max_score,
            "pct": pct,
            "classificacao": _classificar(pct, ALIMENTACAO_CLASSIFICACAO[dom["key"]]),
        })
    total_pct = int(total_score / total_max * 100) if total_max else 0
    class_total = _classificar(total_pct, ALIMENTACAO_CLASSIFICACAO["total"])
    paciente = avaliacao.paciente
    todas_av = list(paciente.avaliacoes_alimentacao.filter(status="concluida").order_by("data"))
    ...
    for i, dom in enumerate(ALIMENTACAO_DOMINIOS):
        mx = len(dom["itens"]) * 3
...
@login_required
def alimentacao_visualizar(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoAlimentacao, uuid=avaliacao_id, paciente__medico=request.user)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("alimentacao_visualizar", avaliacao_id=avaliacao_id, pagina=1)
