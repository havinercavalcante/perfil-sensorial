...
from ..models import Paciente, AvaliacaoVozInfantil, RespostaVozInfantil
from ..data.data_voz_infantil import VOZ_INFANTIL_DOMINIOS, VOZ_INFANTIL_OPCOES, VOZ_INFANTIL_CLASSIFICACAO
from ..services import notificar_terapeuta

TOTAL_PAGINAS = len(VOZ_INFANTIL_DOMINIOS)

_URL_NAMES = {
    "url_form_name": "voz_form",
    "url_publico_name": "voz_publico",
    "url_visualizar_name": "voz_visualizar",
    "url_resultado_name": "voz_resultado",
    "avaliacao_titulo": "Triagem de Voz Infantil",
}


def _calcular_voz_infantil(avaliacao):
    for dom in VOZ_INFANTIL_DOMINIOS:
...
def _processar_post_pagina(request, dom):
    ...
    valores_validos = {o["valor"] for o in VOZ_INFANTIL_OPCOES}
    erros, novas, obs = [], {}, {}
...
def _salvar_respostas(avaliacao, dom_key, novas, obs):
    for numero, valor in novas.items():
        RespostaVozInfantil.objects.update_or_create(
...
@login_required
def nova_avaliacao_voz_infantil(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not hasattr(request.user, 'perfil') or not request.user.perfil.tem_acesso('voz_infantil'):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo Triagem de Voz Infantil.")
        return redirect('detalhe_paciente', paciente_id=paciente_id)
    av = AvaliacaoVozInfantil.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("voz_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def voz_infantil_form(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status == "concluida":
        return redirect("voz_resultado", avaliacao_id=avaliacao_id)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("voz_form", avaliacao_id=avaliacao_id, pagina=1)

    dom = VOZ_INFANTIL_DOMINIOS[pagina - 1]
    numeros = [n for n, _, _reverso in dom["itens"]]
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar_incompleto:
            ...
            if proxima > TOTAL_PAGINAS:
                avaliacao = _calcular_voz_infantil(avaliacao)
                avaliacao.status = "concluida"
                avaliacao.save()
                return redirect("voz_resultado", avaliacao_id=avaliacao_id)
            return redirect("voz_form", avaliacao_id=avaliacao_id, pagina=proxima)
    ...
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html", {
        **_URL_NAMES,
        ...
        "dominio": dom,
        "itens": itens_render,
        "opcoes": VOZ_INFANTIL_OPCOES,
        "pagina": pagina,
        "total": TOTAL_PAGINAS,
        "progresso": int((pagina - 1) / TOTAL_PAGINAS * 100),
        "paginas_dominios": [(i + 1, VOZ_INFANTIL_DOMINIOS[i]["nome"]) for i in range(TOTAL_PAGINAS)],
        "pagina_anterior": pagina - 1 if pagina > 1 else None,
        "itens_faltando": itens_faltando,
    })


def voz_infantil_publico(request, token, pagina):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, token=token)
    if avaliacao.status == "concluida":
        return render(request, "questionario/dashboard/concluido.html")
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("voz_publico", token=token, pagina=1)
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar_incompleto:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                try:
                    notificar_terapeuta(avaliacao.paciente, "voz_infantil", request)
                except Exception:
                    pass
                return render(request, "questionario/dashboard/concluido.html")
            return redirect("voz_publico", token=token, pagina=proxima)
...
@login_required
def voz_infantil_resultado(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status != "concluida":
        return redirect("voz_form", avaliacao_id=avaliacao_id, pagina=avaliacao.pagina_atual)
    resultado = []
    total_score = 0
    total_max = 0
    for dom in VOZ_INFANTIL_DOMINIOS:
        ...
        resultado.append({
            ...
            "max": max_score,
            "pct": pct,
            "classificacao": _classificar(pct, VOZ_INFANTIL_CLASSIFICACAO[dom["key"]]),
        })
    total_pct = int(total_score / total_max * 100) if total_max else 0
    class_total = _classificar(total_pct, VOZ_INFANTIL_CLASSIFICACAO["total"])
    paciente = avaliacao.paciente
    todas_av = list(paciente.avaliacoes_voz_infantil.filter(status="concluida").order_by("data"))
    ...
    for i, dom in enumerate(VOZ_INFANTIL_DOMINIOS):
        mx = len(dom["itens"]) * 3
    ...
    return render(request, "questionario/avaliacoes/voz_infantil_resultado.html", {
        "avaliacao": avaliacao,
        "paciente": paciente,
        "resultado": resultado,
...
@login_required
def voz_infantil_visualizar(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, uuid=avaliacao_id, paciente__medico=request.user)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("voz_visualizar", avaliacao_id=avaliacao_id, pagina=1)
...
@login_required
def voz_infantil_deletar(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, uuid=avaliacao_id, paciente__medico=request.user)
    paciente_uuid = avaliacao.paciente.uuid
    if request.method == "POST":
        avaliacao.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "Avaliação de Triagem de Voz Infantil excluída com sucesso."})
        messages.success(request, "Avaliação de Triagem de Voz Infantil excluída com sucesso.")
    return redirect("detalhe_paciente", paciente_id=paciente_uuid)


@login_required
def salvar_observacoes_voz_infantil(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoVozInfantil, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
