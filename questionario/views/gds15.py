...
from ..models import Paciente, AvaliacaoGDS15, RespostaGDS15
from ..data.data_gds15 import GDS15_ITENS, GDS15_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#C0392B"
_TITULO = "GDS-15 — Depressão Geriátrica"
_MAX    = 15
_URL_NAMES = {
    "url_form_name":       "gds15_form",
    "url_publico_name":    "gds15_publico",
    "url_visualizar_name": "gds15_visualizar",
    "url_resultado_name":  "gds15_resultado",
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaGDS15.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    avaliacao.pont_total = sum(respostas.get(item["numero"], 0) for item in GDS15_ITENS)
    return avaliacao


def _classificar(score):
    for label, mn, mx in GDS15_CORTE:
        if mn <= score <= mx:
            return label
    return GDS15_CORTE[-1][0]
...
@login_required
def nova_avaliacao_gds15(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("gds15"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo GDS-15 — Depressão Geriátrica.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoGDS15.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("gds15_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def gds15_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoGDS15, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("gds15_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def gds15_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGDS15, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("gds15_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_gds15").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/gds15_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": _MAX,
        "classificacao": _classificar(total),
        "corte": GDS15_CORTE, "cor": _COR,
...
@login_required
def gds15_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoGDS15, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def gds15_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGDS15, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "GDS-15 — Depressão Geriátrica excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_gds15(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGDS15, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def gds15_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoGDS15, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            av.save()
            try:
                notificar_terapeuta(av.paciente, "gds15", request)
            except Exception:
                pass
            return render(request, "questionario/dashboard/concluido.html")
...
@login_required
def enviar_email_gds15(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("gds15_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="GDS-15 — Depressão Geriátrica — IntegraMente",
