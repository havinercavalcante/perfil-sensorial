...
from ..models import Paciente, AvaliacaoChecklistDislexia, RespostaChecklistDislexia
from ..data.data_checklist_dislexia import (
    CHECKLIST_DISLEXIA_ITENS, CHECKLIST_DISLEXIA_SUBESCALAS,
    CHECKLIST_DISLEXIA_OPCOES, CHECKLIST_DISLEXIA_CORTE,
)
from ..tasks import enviar_email
from django.templatetags.static import static

CHECKLIST_DISLEXIA_PAGINAS = [
    {"key": "leitura",       "nome": "Leitura",                 "cor": "#E74C3C", "campo": "pont_leitura"},
    {"key": "escrita",       "nome": "Escrita",                 "cor": "#3498DB", "campo": "pont_escrita"},
    {"key": "processamento", "nome": "Processamento Fonológico", "cor": "#27AE60", "campo": "pont_processamento"},
]
TOTAL_PAGINAS = 3

_URL_NAMES = {
    "url_form_name": "checklist_dislexia_form",
    "url_publico_name": "checklist_dislexia_publico",
    "url_visualizar_name": "checklist_dislexia_visualizar",
    "url_resultado_name": "checklist_dislexia_resultado",
    "avaliacao_titulo": "Checklist de Dislexia",
}

_ITENS_POR_SUB = {
    key: [i for i in CHECKLIST_DISLEXIA_ITENS if i["subescala"] == key]
    for key in CHECKLIST_DISLEXIA_SUBESCALAS
}
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaChecklistDislexia.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    total = 0
    for key, sub in CHECKLIST_DISLEXIA_SUBESCALAS.items():
...
def _ctx(avaliacao, pagina_dict, pagina, itens_render, rs, itens_faltando, extra=None):
    ctx = {
        **_URL_NAMES,
        ...
        "dominio": pagina_dict,
        "itens": itens_render,
        "opcoes": CHECKLIST_DISLEXIA_OPCOES,
        "pagina": pagina,
        "total": TOTAL_PAGINAS,
        "progresso": int((pagina - 1) / TOTAL_PAGINAS * 100),
        "paginas_dominios": [(i + 1, CHECKLIST_DISLEXIA_PAGINAS[i]["nome"]) for i in range(TOTAL_PAGINAS)],
...
@login_required
def nova_avaliacao_checklist_dislexia(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not hasattr(request.user, 'perfil') or not request.user.perfil.tem_acesso('checklist_dislexia'):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo Checklist de Dislexia.")
        return redirect('detalhe_paciente', paciente_id=paciente_id)
    av = AvaliacaoChecklistDislexia.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("checklist_dislexia_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def checklist_dislexia_form(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status == "concluida":
        return redirect("checklist_dislexia_resultado", avaliacao_id=avaliacao_id)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("checklist_dislexia_form", avaliacao_id=avaliacao_id, pagina=1)
    pagina_dict = CHECKLIST_DISLEXIA_PAGINAS[pagina - 1]
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                return redirect("checklist_dislexia_resultado", avaliacao_id=avaliacao_id)
            return redirect("checklist_dislexia_form", avaliacao_id=avaliacao_id, pagina=proxima)
...
@login_required
def checklist_dislexia_resultado(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status != "concluida":
        return redirect("checklist_dislexia_form", avaliacao_id=avaliacao_id, pagina=1)
    ...
    max_total = len(CHECKLIST_DISLEXIA_ITENS)
    rastreio_positivo = total >= CHECKLIST_DISLEXIA_CORTE

    paciente = avaliacao.paciente
    todas_av = list(paciente.avaliacoes_checklist_dislexia.filter(status="concluida").order_by("data"))
    outras = [av for av in todas_av if av.id != avaliacao.id]

    return render(request, "questionario/avaliacoes/checklist_dislexia_resultado.html", {
        "avaliacao": avaliacao, "paciente": paciente,
        "resultado": resultado,
        "total": total, "max_total": max_total,
        "corte": CHECKLIST_DISLEXIA_CORTE, "rastreio_positivo": rastreio_positivo,
        ...
        "comparativo_datasets": json.dumps([
            {"label": "Total", "data": [av.pont_total or 0 for av in todas_av],
             "borderColor": "#E74C3C", "backgroundColor": "#E74C3C33", "tension": 0.3}
        ]),
...
@login_required
def checklist_dislexia_visualizar(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, uuid=avaliacao_id, paciente__medico=request.user)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("checklist_dislexia_visualizar", avaliacao_id=avaliacao_id, pagina=1)
    pagina_dict = CHECKLIST_DISLEXIA_PAGINAS[pagina - 1]
    itens_pagina = _itens_da_pagina(pagina_dict)
    numeros = [n for n, _ in itens_pagina]
    qs = list(avaliacao.respostas.filter(numero_item__in=numeros))
...
@login_required
def checklist_dislexia_deletar(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, uuid=avaliacao_id, paciente__medico=request.user)
    paciente_uuid = avaliacao.paciente.uuid
    if request.method == "POST":
        avaliacao.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "Checklist excluído."})
        messages.success(request, "Checklist excluído.")
    return redirect("detalhe_paciente", paciente_id=paciente_uuid)


@login_required
def salvar_observacoes_checklist_dislexia(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
@login_required
def enviar_email_checklist_dislexia(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(
        reverse("checklist_dislexia_publico", kwargs={"token": avaliacao.token, "pagina": 1})
    )
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="Checklist de Dislexia — IntegraMente", message=f"Link: {link}",
...
def checklist_dislexia_publico(request, token, pagina):
    from ..services import notificar_terapeuta
    avaliacao = get_object_or_404(AvaliacaoChecklistDislexia, token=token)
    if avaliacao.status == "concluida":
        return render(request, "questionario/dashboard/concluido.html")
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("checklist_dislexia_publico", token=token, pagina=1)
    pagina_dict = CHECKLIST_DISLEXIA_PAGINAS[pagina - 1]
    itens_pagina = _itens_da_pagina(pagina_dict)
    numeros = [n for n, _ in itens_pagina]
    qs = list(avaliacao.respostas.filter(numero_item__in=numeros))
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                try:
                    notificar_terapeuta(avaliacao.paciente, "checklist_dislexia", request)
                except Exception:
                    pass
                return render(request, "questionario/dashboard/concluido.html")
            return redirect("checklist_dislexia_publico", token=token, pagina=proxima)
