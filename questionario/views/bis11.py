...
from ..models import Paciente, AvaliacaoBIS11, RespostaBIS11
from ..data.data_bis11 import BIS11_ITENS, BIS11_OPCOES, BIS11_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#8E44AD"
_TITULO = "BIS-11 — Impulsividade de Barratt"
_MAX    = 120
_REVERSOS = {9, 20, 30}
_URL_NAMES = {
    "url_form_name":       "bis11_form",
    "url_publico_name":    "bis11_publico",
    "url_visualizar_name": "bis11_visualizar",
    "url_resultado_name":  "bis11_resultado",
...
def _itens_render(respostas_salvas, obs_salvas, obs_render):
    return [
        {"numero": n, "texto": t, "opcoes": BIS11_OPCOES,
          "resposta_salva": respostas_salvas.get(n),
          "observacao_salva": obs_render.get(n, obs_salvas.get(n, ""))}
        for n, t in BIS11_ITENS
    ]


def _processar(request):
    erros, novas, obs = [], {}, {}
    vals = {o["valor"] for o in BIS11_OPCOES}
    for n, _ in BIS11_ITENS:
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaBIS11.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    for n, _ in BIS11_ITENS:
        v = respostas.get(n, 0)
        if n in _REVERSOS:
            max_v = max(o["valor"] for o in BIS11_OPCOES)
            min_v = min(o["valor"] for o in BIS11_OPCOES)
            v = max_v + min_v - v
        total += v
...
def _classificar(score):
    for label, mn, mx in BIS11_CORTE:
        if mn <= score <= mx:
            return label
    return BIS11_CORTE[-1][0]
...
@login_required
def nova_avaliacao_bis11(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("bis11"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo BIS-11 — Impulsividade de Barratt.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoBIS11.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("bis11_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def bis11_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoBIS11, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("bis11_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def bis11_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoBIS11, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("bis11_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_bis11").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/bis11_resultado.html", {
        "avaliacao": av, "paciente": av.paciente, "total": total, "max_total": _MAX,
        "classificacao": _classificar(total), "corte": BIS11_CORTE, "cor": _COR,
...
@login_required
def bis11_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoBIS11, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def bis11_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoBIS11, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "BIS-11 — Impulsividade de Barratt excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_bis11(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoBIS11, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def bis11_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoBIS11, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            _salvar(av, novas, obs); av = _calcular(av); av.status = "concluida"; av.save()
            try: notificar_terapeuta(av.paciente, "bis11", request)
...
@login_required
def enviar_email_bis11(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("bis11_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="BIS-11 — Impulsividade de Barratt — IntegraMente", message=f"Responda em: {link}",
