...
from ..models import Paciente, AvaliacaoGAD7, RespostaGAD7
from ..data.data_gad7 import GAD7_ITENS, GAD7_OPCOES, GAD7_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR   = "#1D6A96"
_TITULO = "GAD-7 — Transtorno de Ansiedade Generalizada"
_URL_NAMES = {
    "url_form_name":       "gad7_form",
    "url_publico_name":    "gad7_publico",
    "url_visualizar_name": "gad7_visualizar",
    "url_resultado_name":  "gad7_resultado",
...
def _itens_render(respostas_salvas, obs_salvas, obs_render):
    return [
        ...
        for n, t in GAD7_ITENS
    ]


def _processar(request):
    erros, novas, obs = [], {}, {}
    for n, _ in GAD7_ITENS:
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaGAD7.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    avaliacao.pont_total = sum(respostas.get(n, 0) for n, _ in GAD7_ITENS)
    return avaliacao


def _classificar(score):
    for label, mn, mx in GAD7_CORTE:
        if mn <= score <= mx:
            return label
    return GAD7_CORTE[-1][0]
...
@login_required
def nova_avaliacao_gad7(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("gad7"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo GAD-7.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoGAD7.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("gad7_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def gad7_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoGAD7, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("gad7_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def gad7_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGAD7, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("gad7_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(av.paciente.avaliacoes_gad7.filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/gad7_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": 21,
        "classificacao": _classificar(total),
        "corte": GAD7_CORTE, "cor": _COR,
        "comparativo_labels": json.dumps([a.data.strftime("%d/%m/%Y") for a in todas]),
        "comparativo_datasets": json.dumps([{"label": "GAD-7", "data": [a.pont_total or 0 for a in todas], "borderColor": _COR, "backgroundColor": _COR + "33", "tension": 0.3}]),
        "tem_comparativo": len(todas) > 1, "outras_avaliacoes": outras,
    })


@login_required
def gad7_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoGAD7, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
...
@login_required
def gad7_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGAD7, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "GAD-7 excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_gad7(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoGAD7, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def gad7_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoGAD7, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            av.save()
            try:
                notificar_terapeuta(av.paciente, "gad7", request)
            except Exception:
                pass
            return render(request, "questionario/dashboard/concluido.html")
...
@login_required
def enviar_email_gad7(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("gad7_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="GAD-7 — IntegraMente", message=f"Responda o questionário em: {link}",
