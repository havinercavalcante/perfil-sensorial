...
_MAX    = 10
_URL_NAMES = {
    "url_form_name":       "risco_suicidio_form",
    "url_publico_name":    "risco_suicidio_publico",
    "url_visualizar_name": "risco_suicidio_visualizar",
    "url_resultado_name":  "risco_suicidio_resultado",
...
def _itens_render(respostas_salvas, obs_salvas, obs_render):
    return [
        ...
        for item in RISCO_SUICIDIO_ITENS
    ]
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaRiscoSuicidio.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    avaliacao.pont_total = sum(respostas.get(item["numero"], 0) for item in RISCO_SUICIDIO_ITENS)
    return avaliacao


def _classificar(score):
    for label, mn, mx in RISCO_SUICIDIO_CORTE:
        if mn <= score <= mx:
            return label
    return RISCO_SUICIDIO_CORTE[-1][0]
...
@login_required
def nova_avaliacao_risco_suicidio(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("risco_suicidio"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo Triagem de Risco de Suicídio.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoRiscoSuicidio.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("risco_suicidio_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def risco_suicidio_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("risco_suicidio_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def risco_suicidio_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("risco_suicidio_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_risco_suicidio").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/risco_suicidio_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": _MAX,
        "classificacao": _classificar(total),
        "corte": RISCO_SUICIDIO_CORTE, "cor": _COR,
...
@login_required
def risco_suicidio_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def risco_suicidio_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "Triagem de Risco de Suicídio excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_risco_suicidio(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def risco_suicidio_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoRiscoSuicidio, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            av.save()
            try:
                notificar_terapeuta(av.paciente, "risco_suicidio", request)
            except Exception:
                pass
            return render(request, "questionario/dashboard/concluido.html")
