...
_URL_NAMES = {
    "url_form_name":       "phq9_form",
    "url_publico_name":    "phq9_publico",
    "url_visualizar_name": "phq9_visualizar",
    "url_resultado_name":  "phq9_resultado",
...
def _itens_render(respostas_salvas, obs_salvas, obs_render):
    return [
        ...
        for n, t in PHQ9_ITENS
    ]


def _processar(request):
    erros, novas, obs = [], {}, {}
    for n, _ in PHQ9_ITENS:
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaPHQ9.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    avaliacao.pont_total = sum(respostas.get(n, 0) for n, _ in PHQ9_ITENS)
    return avaliacao


def _classificar(score):
    for label, mn, mx in PHQ9_CORTE:
        if mn <= score <= mx:
            return label
    return PHQ9_CORTE[-1][0]
...
@login_required
def nova_avaliacao_phq9(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("phq9"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo PHQ-9.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoPHQ9.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("phq9_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def phq9_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoPHQ9, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("phq9_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def phq9_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPHQ9, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("phq9_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(av.paciente.avaliacoes_phq9.filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/phq9_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": 27,
        "classificacao": _classificar(total),
        "corte": PHQ9_CORTE, "cor": _COR,
        "comparativo_labels": json.dumps([a.data.strftime("%d/%m/%Y") for a in todas]),
        "comparativo_datasets": json.dumps([{"label": "PHQ-9", "data": [a.pont_total or 0 for a in todas], "borderColor": _COR, "backgroundColor": _COR + "33", "tension": 0.3}]),
        "tem_comparativo": len(todas) > 1, "outras_avaliacoes": outras,
    })


@login_required
def phq9_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoPHQ9, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
...
@login_required
def phq9_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPHQ9, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "PHQ-9 excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_phq9(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPHQ9, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def phq9_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoPHQ9, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            av.save()
            try:
                notificar_terapeuta(av.paciente, "phq9", request)
            except Exception:
                pass
            return render(request, "questionario/dashboard/concluido.html")
