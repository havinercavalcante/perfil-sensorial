...
from ..models import Paciente, AvaliacaoComportamentoFuncional, RespostaComportamentoFuncional
from ..data.data_comportamento_funcional import COMPORTAMENTO_DOMINIOS, COMPORTAMENTO_OPCOES
from ..services import notificar_terapeuta

# RespostaComportamentoFuncional NÃO tem campo dominio — itens são salvos com numero_item global.
...
def _calcular_comportamento_funcional(avaliacao):
...
def nova_avaliacao_comportamento_funcional(request, paciente_id):
    ...
    if not hasattr(request.user, 'perfil') or not request.user.perfil.tem_acesso('comportamento_funcional'):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo Avaliação de Comportamento Funcional.")
...
def comportamento_funcional_deletar(request, avaliacao_id):
    ...
    if request.method == "POST":
        avaliacao.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "Avaliação de Comportamento Funcional excluída com sucesso."})
        messages.success(request, "Avaliação de Comportamento Funcional excluída com sucesso.")
