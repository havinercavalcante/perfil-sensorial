...
from ..models import Paciente, AvaliacaoMASC, RespostaMASC
from ..data.data_masc import MASC_ITENS, MASC_SUBESCALAS, MASC_OPCOES, MASC_CORTE
from ..services import notificar_terapeuta

MASC_PAGINAS = [
    {"key": "sint_fisicos", "nome": "Sintomas Físicos",               "cor": "#C0392B", "campo": "pont_sint_fisicos"},
    {"key": "anx_social",   "nome": "Ansiedade Social",               "cor": "#9B59B6", "campo": "pont_anx_social"},
    {"key": "evit_danos",   "nome": "Evitação de Danos",              "cor": "#27AE60", "campo": "pont_evit_danos"},
    {"key": "sep_panico",   "nome": "Ansiedade de Separação / Pânico","cor": "#3498DB", "campo": "pont_sep_panico"},
]
TOTAL_PAGINAS = len(MASC_PAGINAS)

_ITENS_POR_SUB = {key: [i for i in MASC_ITENS if i["subescala"] == key] for key in MASC_SUBESCALAS}

_URL_NAMES = {
    "url_form_name": "masc_form", "url_publico_name": "masc_publico",
    "url_resultado_name": "masc_resultado", "url_visualizar_name": "masc_visualizar",
    "avaliacao_titulo": "MASC — Ansiedade Multidimensional",
}
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaMASC.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    total = 0
    for key, sub in MASC_SUBESCALAS.items():
...
def _ctx(avaliacao, pagina_dict, pagina, itens_render, rs, itens_faltando, extra=None):
    ctx = {
        **_URL_NAMES, "avaliacao": avaliacao, "paciente": avaliacao.paciente,
        "dominio": pagina_dict, "itens": itens_render, "opcoes": MASC_OPCOES,
        "pagina": pagina, "total": TOTAL_PAGINAS,
        "progresso": int((pagina - 1) / TOTAL_PAGINAS * 100),
        "paginas_dominios": [(i + 1, MASC_PAGINAS[i]["nome"]) for i in range(TOTAL_PAGINAS)],
...
@login_required
def nova_avaliacao_masc(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not hasattr(request.user, 'perfil') or not request.user.perfil.tem_acesso('masc'):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível no seu plano."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo MASC.")
        return redirect('detalhe_paciente', paciente_id=paciente_id)
    av = AvaliacaoMASC.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("masc_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def masc_form(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoMASC, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status == "concluida":
        return redirect("masc_resultado", avaliacao_id=avaliacao_id)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("masc_form", avaliacao_id=avaliacao_id, pagina=1)
    pagina_dict = MASC_PAGINAS[pagina - 1]
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                return redirect("masc_resultado", avaliacao_id=avaliacao_id)
            return redirect("masc_form", avaliacao_id=avaliacao_id, pagina=proxima)
...
@login_required
def masc_resultado(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoMASC, uuid=avaliacao_id, paciente__medico=request.user)
    if avaliacao.status != "concluida":
        return redirect("masc_form", avaliacao_id=avaliacao_id, pagina=1)
    resultado = []
    for key, sub in MASC_SUBESCALAS.items():
        ...
        resultado.append({
            ...
            "classificacao": _classificar(score, MASC_CORTE.get(key, [("Normal", 0, 9999)])),
        })
    total = avaliacao.pont_total or 0
    paciente = avaliacao.paciente
    todas_av = list(paciente.avaliacoes_masc.filter(status="concluida").order_by("data"))
    outras = [av for av in todas_av if av.id != avaliacao.id]
    return render(request, "questionario/avaliacoes/masc_resultado.html", {
        "avaliacao": avaliacao, "paciente": paciente, "resultado": resultado,
        "total": total, "max_total": 117,
        "class_total": _classificar(total, MASC_CORTE["total"]),
        "pct_total": int(total / 117 * 100),
        ...
        "comparativo_datasets": json.dumps([
            ...
            for s in MASC_SUBESCALAS.values()
        ]),
...
@login_required
def masc_visualizar(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoMASC, uuid=avaliacao_id, paciente__medico=request.user)
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("masc_visualizar", avaliacao_id=avaliacao_id, pagina=1)
    pagina_dict = MASC_PAGINAS[pagina - 1]
    itens_pagina = _ITENS_POR_SUB[pagina_dict["key"]]
    numeros = [i["numero"] for i in itens_pagina]
    qs = list(avaliacao.respostas.filter(numero_item__in=numeros))
...
@login_required
def masc_deletar(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoMASC, uuid=avaliacao_id, paciente__medico=request.user)
    paciente_uuid = avaliacao.paciente.uuid
    if request.method == "POST":
        avaliacao.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "MASC excluído com sucesso."})
        messages.success(request, "MASC excluído com sucesso.")
    return redirect("detalhe_paciente", paciente_id=paciente_uuid)


@login_required
def salvar_observacoes_masc(request, avaliacao_id):
    avaliacao = get_object_or_404(AvaliacaoMASC, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
@login_required
def enviar_email_masc(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("masc_publico", kwargs={"token": avaliacao.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        send_mail(subject="MASC — IntegraMente", message=f"Link: {link}", from_email=None, recipient_list=[email_dest], html_message=html, fail_silently=False)
...
def masc_publico(request, token, pagina):
    avaliacao = get_object_or_404(AvaliacaoMASC, token=token)
    if avaliacao.status == "concluida":
        return render(request, "questionario/dashboard/concluido.html")
    if pagina < 1 or pagina > TOTAL_PAGINAS:
        return redirect("masc_publico", token=token, pagina=1)
    pagina_dict = MASC_PAGINAS[pagina - 1]
    itens_pagina = _ITENS_POR_SUB[pagina_dict["key"]]
    numeros = [i["numero"] for i in itens_pagina]
    qs = list(avaliacao.respostas.filter(numero_item__in=numeros))
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            if proxima > TOTAL_PAGINAS:
                ...
                avaliacao.save()
                try:
                    notificar_terapeuta(avaliacao.paciente, "masc", request)
                except Exception:
                    pass
                return render(request, "questionario/dashboard/concluido.html")
            return redirect("masc_publico", token=token, pagina=proxima)
    itens_render = [{"numero": i["numero"], "texto": i["texto"], "resposta_salva": rs.get(i["numero"]), "observacao_salva": obs_render.get(i["numero"], os_.get(i["numero"], ""))} for i in itens_pagina]
