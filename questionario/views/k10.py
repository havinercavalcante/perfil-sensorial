...
from ..models import Paciente, AvaliacaoK10, RespostaK10
from ..data.data_k10 import K10_ITENS, K10_OPCOES, K10_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#1D6A96"
_TITULO = "K-10 — Escala de Kessler"
_MAX    = 50
_REVERSOS = set()
_URL_NAMES = {
    "url_form_name":       "k10_form",
    "url_publico_name":    "k10_publico",
    "url_visualizar_name": "k10_visualizar",
    "url_resultado_name":  "k10_resultado",
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaK10.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    for n, _ in K10_ITENS:
        v = respostas.get(n, 0)
        if n in _REVERSOS:
            max_v = max(o["valor"] for o in K10_OPCOES)
            min_v = min(o["valor"] for o in K10_OPCOES)
            v = max_v + min_v - v
        total += v
...
def _classificar(score):
    for label, mn, mx in K10_CORTE:
        if mn <= score <= mx:
            return label
    return K10_CORTE[-1][0]
...
@login_required
def nova_avaliacao_k10(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("k10"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo K-10 — Escala de Kessler.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoK10.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("k10_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def k10_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoK10, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("k10_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def k10_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoK10, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("k10_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_k10").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/k10_resultado.html", {
        "avaliacao": av, "paciente": av.paciente, "total": total, "max_total": _MAX,
        "classificacao": _classificar(total), "corte": K10_CORTE, "cor": _COR,
...
@login_required
def k10_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoK10, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def k10_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoK10, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "K-10 — Escala de Kessler excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_k10(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoK10, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def k10_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoK10, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            _salvar(av, novas, obs); av = _calcular(av); av.status = "concluida"; av.save()
            try: notificar_terapeuta(av.paciente, "k10", request)
...
@login_required
def enviar_email_k10(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("k10_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="K-10 — Escala de Kessler — IntegraMente", message=f"Responda em: {link}",
