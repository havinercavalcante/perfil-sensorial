...
from ..models import Paciente, AvaliacaoAUDIT, RespostaAUDIT
from ..data.data_audit import AUDIT_ITENS, AUDIT_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#E67E22"
_TITULO = "AUDIT — Uso de Álcool"
_MAX    = 40
_URL_NAMES = {
    "url_form_name":       "audit_form",
    "url_publico_name":    "audit_publico",
    "url_visualizar_name": "audit_visualizar",
    "url_resultado_name":  "audit_resultado",
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaAUDIT.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    avaliacao.pont_total = sum(respostas.get(item["numero"], 0) for item in AUDIT_ITENS)
    return avaliacao


def _classificar(score):
    for label, mn, mx in AUDIT_CORTE:
        if mn <= score <= mx:
            return label
    return AUDIT_CORTE[-1][0]
...
@login_required
def nova_avaliacao_audit(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("audit"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo AUDIT — Uso de Álcool.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoAUDIT.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("audit_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def audit_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoAUDIT, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("audit_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def audit_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoAUDIT, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("audit_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_audit").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/audit_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": _MAX,
        "classificacao": _classificar(total),
        "corte": AUDIT_CORTE, "cor": _COR,
...
@login_required
def audit_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoAUDIT, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def audit_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoAUDIT, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "AUDIT — Uso de Álcool excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_audit(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoAUDIT, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def audit_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoAUDIT, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            av.save()
            try:
                notificar_terapeuta(av.paciente, "audit", request)
            except Exception:
                pass
            return render(request, "questionario/dashboard/concluido.html")
...
@login_required
def enviar_email_audit(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("audit_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="AUDIT — Uso de Álcool — IntegraMente",
