...
def nova_avaliacao_sono(request, paciente_id):
    ...
    return redirect("sono_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def sono_form(request, avaliacao_id, pagina):
    avaliacao = get_object_or_404(AvaliacaoSono, uuid=avaliacao_id, paciente__medico=request.user)
...
def sono_resultado(request, avaliacao_id):
    ...
    paciente = avaliacao.paciente
    todas_av = list(paciente.avaliacoes_sono.filter(status="concluida").order_by("data"))
    ...
    cores_linha = ["#2E7D6B", "#3E73D1", "#E8793A", "#9B59B6", "#E8B84B", "#C0392B", "#16A085"]
    comparativo_datasets = []
    for i, dom in enumerate(SONO_DOMINIOS):
    ...
    total_max_sono = sum(len(dom["itens"]) * max_por_item for dom in SONO_DOMINIOS)
    if total_max_sono:
        comparativo_datasets.append({"label": "Total", "data": [round((av.pont_total or 0) / total_max_sono * 100) for av in todas_av], "borderColor": cores_linha[6], "backgroundColor": cores_linha[6] + "33", "tension": 0.3})
