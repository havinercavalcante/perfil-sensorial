...
from ..models import Paciente, AvaliacaoPAS, RespostaPAS
from ..data.data_pas import PAS_ITENS, PAS_OPCOES, PAS_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#D35400"
_TITULO = "PAS — Ansiedade Pré-Escolar"
_MAX    = 112
_REVERSOS = set()
_URL_NAMES = {
    "url_form_name":       "pas_form",
    "url_publico_name":    "pas_publico",
    "url_visualizar_name": "pas_visualizar",
    "url_resultado_name":  "pas_resultado",
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaPAS.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    for n, _ in PAS_ITENS:
        v = respostas.get(n, 0)
        if n in _REVERSOS:
            max_v = max(o["valor"] for o in PAS_OPCOES)
            min_v = min(o["valor"] for o in PAS_OPCOES)
            v = max_v + min_v - v
        total += v
...
def _classificar(score):
    for label, mn, mx in PAS_CORTE:
        if mn <= score <= mx:
            return label
    return PAS_CORTE[-1][0]
...
@login_required
def nova_avaliacao_pas(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("pas"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo PAS — Ansiedade Pré-Escolar.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoPAS.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("pas_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def pas_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoPAS, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("pas_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def pas_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPAS, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("pas_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_pas").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/pas_resultado.html", {
        "avaliacao": av, "paciente": av.paciente, "total": total, "max_total": _MAX,
        "classificacao": _classificar(total), "corte": PAS_CORTE, "cor": _COR,
...
@login_required
def pas_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoPAS, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def pas_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPAS, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "PAS — Ansiedade Pré-Escolar excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_pas(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoPAS, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def pas_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoPAS, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            _salvar(av, novas, obs); av = _calcular(av); av.status = "concluida"; av.save()
            try: notificar_terapeuta(av.paciente, "pas", request)
...
@login_required
def enviar_email_pas(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("pas_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="PAS — Ansiedade Pré-Escolar — IntegraMente", message=f"Responda em: {link}",
