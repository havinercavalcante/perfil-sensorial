...
from ..models import Paciente, AvaliacaoIAR, RespostaIAR
from ..data.data_iar import IAR_ITENS, IAR_OPCOES, IAR_CORTE
from ..services import notificar_terapeuta
from ..tasks import enviar_email
from django.templatetags.static import static

_COR    = "#16A085"
_TITULO = "IAR — Inventário de Ansiedade Infantil"
_MAX    = 60
_REVERSOS = set()
_URL_NAMES = {
    "url_form_name":       "iar_form",
    "url_publico_name":    "iar_publico",
    "url_visualizar_name": "iar_visualizar",
    "url_resultado_name":  "iar_resultado",
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaIAR.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    for n, _ in IAR_ITENS:
        v = respostas.get(n, 0)
        if n in _REVERSOS:
            max_v = max(o["valor"] for o in IAR_OPCOES)
            min_v = min(o["valor"] for o in IAR_OPCOES)
            v = max_v + min_v - v
        total += v
...
def _classificar(score):
    for label, mn, mx in IAR_CORTE:
        if mn <= score <= mx:
            return label
    return IAR_CORTE[-1][0]
...
@login_required
def nova_avaliacao_iar(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("iar"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo IAR — Inventário de Ansiedade Infantil.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoIAR.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("iar_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def iar_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoIAR, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("iar_resultado", avaliacao_id=avaliacao_id)
...
_OPCOES_LABEL = {0: "Nunca", 1: "Às vezes", 2: "Sempre"}

_COR_RESP = {
    0: ("resp-nunca",    "Nunca"),
    1: ("resp-as-vezes", "Às vezes"),
    2: ("resp-sempre",   "Sempre"),
}

_INTERPRETACAO = {
    "Ansiedade mínima":    "Pontuação dentro da faixa esperada. Não há indicativos de ansiedade clínica no momento.",
    "Ansiedade leve":      "Presença de alguns sintomas ansiosos. Recomenda-se monitoramento e estratégias de manejo.",
    "Ansiedade moderada":  "Nível de ansiedade com impacto funcional relevante. Avaliação clínica aprofundada e intervenção são indicadas.",
    "Ansiedade grave":     "Sintomas ansiosos intensos com provável prejuízo significativo. Encaminhamento e intervenção prioritários.",
}


@login_required
def iar_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoIAR, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("iar_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    classificacao = _classificar(total)

    respostas = {r.numero_item: r.valor for r in av.respostas.all()}
    itens_ctx = []
    for n, texto in IAR_ITENS:
        valor = respostas.get(n, 0)
        css, label = _COR_RESP.get(valor, ("resp-nunca", "Nunca"))
        itens_ctx.append({"num": n, "texto": texto, "valor": valor, "label": label, "css": css})

    itens_destaque = [i for i in itens_ctx if i["valor"] == 2]

    todas = list(av.paciente.avaliacoes_iar.filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]

    return render(request, "questionario/avaliacoes/iar_resultado.html", {
        "avaliacao": av, "paciente": av.paciente,
        "total": total, "max_total": _MAX,
        "pct": round(total / _MAX * 100),
        "classificacao": classificacao,
        "interpretacao": _INTERPRETACAO.get(classificacao, ""),
        "corte": IAR_CORTE, "cor": _COR,
        "itens": itens_ctx,
...
@login_required
def iar_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoIAR, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def iar_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoIAR, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "IAR — Inventário de Ansiedade Infantil excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_iar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoIAR, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def iar_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoIAR, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            _salvar(av, novas, obs); av = _calcular(av); av.status = "concluida"; av.save()
            try: notificar_terapeuta(av.paciente, "iar", request)
...
@login_required
def enviar_email_iar(request, avaliacao_id):
    ...
    link = request.build_absolute_uri(reverse("iar_publico", kwargs={"token": av.token, "pagina": 1}))
    html = render_to_string("questionario/emails/email_link_avaliacao.html", {"paciente": paciente, "link": link})
    try:
        enviar_email.delay(subject="IAR — Inventário de Ansiedade Infantil — IntegraMente", message=f"Responda em: {link}",
