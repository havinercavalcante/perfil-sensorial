...
_REVERSOS = set()
_URL_NAMES = {
    "url_form_name":       "hama_form",
    "url_publico_name":    "hama_publico",
    "url_visualizar_name": "hama_visualizar",
    "url_resultado_name":  "hama_resultado",
...
def _itens_render(respostas_salvas, obs_salvas, obs_render):
    return [
        {"numero": n, "texto": t, "opcoes": HAMA_OPCOES,
          "resposta_salva": respostas_salvas.get(n),
          "observacao_salva": obs_render.get(n, obs_salvas.get(n, ""))}
        for n, t in HAMA_ITENS
    ]


def _processar(request):
    erros, novas, obs = [], {}, {}
    vals = {o["valor"] for o in HAMA_OPCOES}
    for n, _ in HAMA_ITENS:
...
def _salvar(avaliacao, novas, obs):
    for n, v in novas.items():
        RespostaHAMA.objects.update_or_create(
...
def _calcular(avaliacao):
    ...
    for n, _ in HAMA_ITENS:
        v = respostas.get(n, 0)
        if n in _REVERSOS:
            max_v = max(o["valor"] for o in HAMA_OPCOES)
            min_v = min(o["valor"] for o in HAMA_OPCOES)
            v = max_v + min_v - v
        total += v
...
def _classificar(score):
    for label, mn, mx in HAMA_CORTE:
        if mn <= score <= mx:
            return label
    return HAMA_CORTE[-1][0]
...
@login_required
def nova_avaliacao_hama(request, paciente_id):
    paciente = get_object_or_404(Paciente, uuid=paciente_id, medico=request.user)
    if not request.user.perfil.tem_acesso("hama"):
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": False, "error": "Módulo não disponível."}, status=403)
        messages.error(request, "Você não tem acesso ao módulo HAM-A — Ansiedade de Hamilton.")
        return redirect("detalhe_paciente", paciente_id=paciente_id)
    av = AvaliacaoHAMA.objects.create(paciente=paciente, token=str(uuid.uuid4()))
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse({"ok": True, "uuid": str(av.uuid)})
    return redirect("hama_form", avaliacao_id=av.uuid, pagina=1)


@login_required
def hama_form(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoHAMA, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status == "concluida":
        return redirect("hama_resultado", avaliacao_id=avaliacao_id)
...
@login_required
def hama_resultado(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoHAMA, uuid=avaliacao_id, paciente__medico=request.user)
    if av.status != "concluida":
        return redirect("hama_form", avaliacao_id=avaliacao_id, pagina=1)
    total = av.pont_total or 0
    todas = list(getattr(av.paciente, "avaliacoes_hama").filter(status="concluida").order_by("data"))
    outras = [a for a in todas if a.id != av.id]
    return render(request, "questionario/avaliacoes/hama_resultado.html", {
        "avaliacao": av, "paciente": av.paciente, "total": total, "max_total": _MAX,
        "classificacao": _classificar(total), "corte": HAMA_CORTE, "cor": _COR,
...
@login_required
def hama_visualizar(request, avaliacao_id, pagina):
    av = get_object_or_404(AvaliacaoHAMA, uuid=avaliacao_id, paciente__medico=request.user)
    qs = list(av.respostas.all())
    return render(request, "questionario/avaliacoes/avaliacao_dominio_form.html",
...
@login_required
def hama_deletar(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoHAMA, uuid=avaliacao_id, paciente__medico=request.user)
    pid = av.paciente.uuid
    if request.method == "POST":
        av.delete()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "message": "HAM-A — Ansiedade de Hamilton excluído com sucesso."})
    return redirect("detalhe_paciente", paciente_id=pid)


@login_required
def salvar_observacoes_hama(request, avaliacao_id):
    av = get_object_or_404(AvaliacaoHAMA, uuid=avaliacao_id, paciente__medico=request.user)
    if request.method == "POST":
...
def hama_publico(request, token, pagina):
    av = get_object_or_404(AvaliacaoHAMA, token=token)
    if av.status == "concluida":
    ...
    if request.method == "POST":
        ...
        if erros and not confirmar:
            ...
            _salvar(av, novas, obs); av = _calcular(av); av.status = "concluida"; av.save()
            try: notificar_terapeuta(av.paciente, "hama", request)
