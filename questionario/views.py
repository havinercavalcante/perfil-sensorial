...
def detalhe_paciente(request, paciente_id):
    ...
    return render(request, "questionario/detalhe_paciente.html", {
        ...
        "avaliacoes_spm": _build_lista_com_link(paciente.avaliacoes_spm.all(), request, "spm_publico"),
    ...
    if request.method == "POST":
        ...
        if erros:
            ...
            if pagina < total:
                ...
                avaliacao.save()
                try:
                    _notificar_terapeuta(avaliacao.paciente, "spm", request)
                except Exception:
                    pass
                return render(request, "questionario/concluido.html")
        respostas_salvas.update(respostas_novas)
...
def gerar_link(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse({"ok": True, "link": link, "tipo_display": convite.get_tipo_display()})
...
def iniciar_avaliacao(request, token):
    ...
    label, icon = _TIPO_INFO.get(convite.tipo, (convite.get_tipo_display(), ""))
    return render(request, "questionario/iniciar_avaliacao.html", {
        "convite": convite,
        "tipo_label": label,
        "tipo_icon": icon,
